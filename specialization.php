...
{
    $sql = $pdo->prepare("SELECT doctors.id as id, specialization FROM `specializations` 
    ...
    while ($row = $sql->fetch()) {
        echo "
            <div class=\"container__radio\">
                <input required id=\"specialization-" . $row['id'] . "\" type=\"radio\" name=\"specialization\" value=\"" . $row['specialization'] . "\"/>
...
<main class="main">
    ...
    <form class="container" action="doctor.php" method="post">
        ...
        //Передача данных введенных на предыдущих страницах (index, city и hospital) через post
